...
export const EVENT_CONFIG = {
  twitch_raid: {
    ...
    chatHtml: (data, html) => html`<strong>${data.from_display_name}</strong> nos hizo raid con <strong>${data.viewer_count}</strong> espectadores!`,
  ...
  twitch_subscribe: {
    ...
    chatHtml: (data, html) => {
      ...
      return html`<strong>${data.display_name}</strong> se suscribió en Tier <strong>${data.tier}</strong>${regalo}!`;
    },
    toastText: (data) => {
      const regalo = data.is_gift ? ' (Regalo)' : '';
  ...
  twitch_sub_gift: {
    ...
    chatHtml: (data, html) => {
      ...
      return html`<strong>${donante}</strong> regaló <strong>${data.total}</strong> subs de Tier <strong>${data.tier}</strong>${acum}!`;
    },
    toastText: (data) => {
  ...
  twitch_sub_message: {
    ...
    chatHtml: (data, html) => {
      ...
      return html`<strong>${data.display_name}</strong> se resuscribió por <strong>${data.cumulative_months}</strong> meses${rachaStr}!${msgStr}`;
    },
    toastText: (data) => {
      const msgStr = data.message ? `: "${data.message}"` : '.';
      const rachaStr = data.streak_months ? ` (Racha: ${data.streak_months} meses)` : '';
  ...
  twitch_cheer: {
    ...
    chatHtml: (data, html) => {
      ...
      return html`<strong>${cheerDonante}</strong> envió <strong>${data.bits}</strong> bits!${cheerMsg}`;
    },
    toastText: (data) => {
      const cheerDonante = data.is_anonymous ? 'Anónimo' : data.display_name;
      const cheerMsg = data.message ? `: "${data.message}"` : '.';
  ...
  twitch_points_redeem: {
    ...
    chatHtml: (data, html) => {
      ...
      return html`<strong>${data.display_name}</strong> canjeó <strong>${data.reward_title}</strong> por <strong>${data.reward_cost}</strong> puntos!${inputStr}`;
    },
    toastText: (data) => {
      const inputStr = data.user_input ? ` ("${data.user_input}")` : '';
  ...
  prediction_begin: {
    ...
    chatHtml: (data, html) => html`Predicción iniciada: "<strong>${data.title}</strong>" - Opciones: ${data.outcomes.join(', ')}`,
  ...
  twitch_ban: {
    ...
    chatHtml: (data, html) => {
      ...
      return html`<strong>${data.display_name}</strong> sancionado (${tipo}) por <strong>${data.moderator_name}</strong>${razon}.`;
    },
    toastText: (data) => {
      const tipo = data.permanent ? 'Baneo permanente' : `Timeout de ${data.duration_seconds}s`;
      const razon = data.reason ? ` (Razón: "${data.reason}")` : '';
  ...
  twitch_unban: {
    ...
    chatHtml: (data, html) => html`<strong>${data.display_name}</strong> desbaneado por <strong>${data.moderator_name}</strong>.`,
    toastText: (data) => `${data.display_name} fue desbaneado por el moderador ${data.moderator_name}.`
  },
  twitch_chat_clear: {
    icon: 'fa-trash-can',
    sysClassName: 'sys-mod',
  ...
  twitch_chat_clear_user: {
    ...
    chatHtml: (data, html) => html`Los mensajes de <strong>${data.display_name}</strong> fueron eliminados por un moderador.`,
